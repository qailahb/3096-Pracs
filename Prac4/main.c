...
uint32_t TIM2_Ticks = (uint32_t) (TIM2CLK/(NS*F_SIGNAL)); 	// Calculation of TIM2_Ticks
uint32_t DestinationAddress = (uint32_t) &(TIM3->CCR3); 	// Writes new LUT values
...
int main(void)
{
  ...
  // TODO: Start DMA in IT mode on TIM2->CH1; Source is LUT and Destination is TIM3->CCR3; start with Sine LUT
...
void EXTI0_1_IRQHandler(void) {
    ...
    if (tick - lastTick > debounceDelay) {
        ...
        // Changes the source address of the DMA based on the current waveform
        switch (buttonPushed) {
            case 0:
                ...
                lcd_putstring("Sawtooth");	// Prints to LCD screen
                buttonPushed = 1;			// Sets parameter value to the switch
                break;
            case 1:
